...
[Group("st", "Storyteller commands")]
public class StorytellerCommandSet : InteractionModuleBase<SocketInteractionContext>
{
    private readonly InteractionService _interactionService;
    private readonly IServiceProvider _provider;
    public StorytellerCommandSet(InteractionService interactionService, IServiceProvider provider)
    {
    ...
    [SlashCommand("asay", "Send a message as \"Ambient\" (descriptive text)", ignoreGroupNames:true)]
    public async Task AmbientSay(string text)
    {
        SayAs("ambient", $"*{text}*");
    }

    [SlashCommand("npc-save", "Save an impersionation as a preset NPC with custom PFP")]
    ...
    private static async Task<DiscordWebhookClient> GetChannelWebhook(SocketTextChannel? channel)
    {
        ...
        if (channel.GetWebhooksAsync().Result.Count(x => x.Name.StartsWith("NPC_")) == 0)
        {
